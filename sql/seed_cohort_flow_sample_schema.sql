...
-- IMPORTANT (Hosted DBs / Railway):
--   1) Import schema into your hosted DB: sql/timetable_schema.sql
--   2) Apply migration: sql/migrations/2026_03_26_year_semester_cohorts.sql
--   3) Import sample faculty + courses (recommended): sql/seed_sample_data_schema.sql
--   4) Then run THIS file (it does NOT select a database).
...
-- - It works best if you import `sql/seed_sample_data_schema.sql` first (faculty + courses).
